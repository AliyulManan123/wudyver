...
import CryptoJS from 'crypto-js';
 ...
 class StartNestAPI {
  ...
  async image({
    ...
    try {
      const { signature } = this._authHeader();
      const { data } = await this.axiosInstance.post('/api/image-generator', {
  ...
  async chat({ prompt, messages, model = 'gpt-3.5-turbo', stream = false, ...rest }) {
    let chatMessages;
    ...
    try {
      ...
      let apiUrl;
      let requestJson;
      const headers = { "Authorization": signature };
      ...
      const { data } = await this.axiosInstance.post(apiUrl, requestJson, { headers });
      ...
      if (stream) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            // Error parsing JSON in stream will no longer be logged by default
          }
        }
