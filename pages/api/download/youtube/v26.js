...
class Cnvmp3Converter {
  ...
  async download({ url, fmt = 1, quality = 128 }, maxTries = 10, delayMs = 2000) {
    ...
    if (!url || typeof url !== 'string' || !url.includes('youtu')) {
      return {
        success: false,
        code: 400,
        result: { error: 'Invalid YouTube URL provided.' }
      };
    }
    ...
    if (![0, 1].includes(fmt)) {
      return {
        success: false,
        code: 400,
        result: { error: 'Invalid format. Format must be 0 (video) or 1 (audio).' }
      };
    }
    ...
    if (q === null) {
      return {
        ...
        result: {
          error: fmt === 1
            ? 'Invalid audio bitrate. Supported bitrates: 96-320 kbps.'
            : 'Invalid video resolution. Supported resolutions: 144, 360, 480, 720, 1080.'
        }
      };
    }
      ...
      try {
        ...
        if (!dlink || typeof dlink !== 'string' || dlink.trim() === '') {
          ...
          return {
            success: false,
            code: 422,
            result: { error: 'Download link not found or invalid after multiple attempts.' }
          };
        }
