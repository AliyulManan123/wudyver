...
class YoutubeDownloader {
  ...
  async getFastytServer() {
    try {
      ...
      const { data } = await axios.get('https://balancer.fastytcdn.com/get-server', {
    ...
    try {
      ...
      $('table.list tbody tr').each((_, el) => {
        ...
        if (dlUrl) {
          ...
          availableQualities.push({ quality: cleanQ, size, url: dlUrl, hasAudio });
        }
      });
      ...
      if (!selectedQuality) {
        if (quality.includes('p')) {
          ...
          selectedQuality = availableQualities
            .filter(q => q.quality.match(/\d+p/))
            .sort((a, b) => parseInt(a.quality) - parseInt(b.quality))
            .find(q => parseInt(q.quality) >= target);
        } else if (quality === 'm4a') {
