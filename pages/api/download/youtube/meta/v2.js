...
class WpsMetadata {
  ...
  generateAuth({ method, uri, timestamp, md5Content, nonce }) {
  ...
  async metadata({ url }) {
    try {
      ...
      const res = await this.axios.post(endpoint, { url }, {
        headers: { Authorization: auth }
      });

      return res.data;
    } catch (e) {
      return { code: -1, error: e.message || e };
    }
  }
}
