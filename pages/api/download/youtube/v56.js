...
class EasyTubeDownloader {
    ...
    try {
      ...
      return $('.converter-results__card').map((_, el) => {
        ...
        const formats = $el.find('table tbody tr').map((_, tr) => {
          ...
          return { quality, format: formatId };
        }).get();

        return { title, thumb, videoId, formats };
      }).get();
    } catch (e) {
  ...
  async download({ url, format = 'mp4', quality = '240p' }) {
    try {
      const videoId = this.extractVideoId(url);
      if (!videoId) throw new Error('Invalid YouTube URL');

      const finalFormat = format === 'mp3' && !quality
        ? 'mp3'
        : `${format}_${quality}`;
  ...
  const downloader = new EasyTubeDownloader();
    ...
    switch (action) {
      case 'search':
        ...
        result = await downloader.search({ query: params.query });
        break;
