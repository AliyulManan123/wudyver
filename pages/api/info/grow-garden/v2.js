...
class GardenStockChecker {
  ...
  async check() {
    ...
    try {
      ...
      const { data } = await client.get(`${this.api.base}${this.api.endpoints.stock()}`, {
      ...
      if (!payload) {
        ...
        return {
          success: false,
          code: 500,
          result: { error: 'Data stok Grow a Garden kosong. Mohon periksa kembali.' }
        };
      }
