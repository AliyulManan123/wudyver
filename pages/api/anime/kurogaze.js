  ...
  async details(url) {
    ...
    if (!url) {
      ...
      return {
        success: false,
        code: 400,
        result: { error: 'URL Kurogaze tidak diberikan.' }
      };
    }
  ...
  async search(keyword = '', page = 1) {
    ...
    if (!keyword) {
      ...
      return {
        success: false,
        code: 400,
        result: { error: 'Kata kunci pencarian tidak boleh kosong.' }
      };
    }
    ...
    try {
      ...
      if (!articles.length) {
        ...
        return {
          success: false,
          code: 404,
          result: { error: `Anime dengan kata kunci "${keyword}" tidak ditemukan.` }
        };
      }
      ...
      const pageNumbers = $('ul.pagination li')
        .map((em, el) => parseInt($(el).text().trim())).get().filter(Boolean);
      const totalPages = Math.max(...pageNumbers, page);
      const hasNextPage = $('ul.pagination li').filter((em, el) =>
        $(el).text().trim() === 'Next' && !$(el).hasClass('disabled')
      ).length > 0;
...
export default async function handler(req, res) {
  ...
  const {
    ...
    query,
    url,
    page // Menambahkan parameter page untuk pencarian
    ...
    switch (action) {
      case "search":
        ...
        if (query) {
          ...
          return res.status(400).json({
            success: false,
            code: 400,
            result: { error: "Parameter 'query' diperlukan untuk aksi 'search'." }
          });
        }

      case "details": // Menggunakan 'details' sesuai nama metode di kelas
        // Memeriksa apakah parameter 'url' ada
        if (url) {
          ...
          return res.status(400).json({
            success: false,
            code: 400,
            result: { error: "Parameter 'url' diperlukan untuk aksi 'details'." }
          });
        }

      case "download": // Menggunakan 'download' untuk mengekstrak link unduhan dari detail
        // Memeriksa apakah parameter 'url' ada
        if (url) {
          console.log(`[API Route] Menerima permintaan 'download' untuk URL: '${url}'`);
          const detailResult = await kuroGrave.details(url);
          if (detailResult.success) {
            // Mengembalikan hanya link unduhan jika detail berhasil diambil
            ...
            // Mengembalikan error jika pengambilan detail gagal
          ...
          return res.status(400).json({
            success: false,
            code: 400,
            result: { error: "Parameter 'url' diperlukan untuk aksi 'download'." }
          });
        }
      ...
      default:
        ...
        return res.status(400).json({
          success: false,
          code: 400,
          result: { error: "Aksi tidak valid. Pilihan: 'search', 'details', 'download', 'schedule', 'ongoing'." }
        });
    }
  } catch (error) {
