...
class QrCodeGenerator {
  async generateQRCode(data, options = {}) {
    ...
    const {
      ...
      size = "Medium",
      color = "#000000",
      istransparent = false, // Changed default to boolean
      backcolor = "#ffffff"
    } = options;
    ...
    const payload = {
      ...
      settings: {
        ...
        size: size,
        color: color,
        istransparent: typeof istransparent === 'boolean' ? istransparent : String(istransparent).toLowerCase() === 'true', // Ensure boolean
        backcolor: backcolor
      },
      output: {
        method: "Base64"
    ...
    try {
      const response = await axios.post("https://qrcode.tec-it.com/API/QRCode", payload, {
        ...
        responseType: 'text' // API returns base64 string directly
      });
