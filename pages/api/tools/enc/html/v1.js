...
class HtmlEncryptor {
  ...
  async encrypt({ html, level }) {
    try {
      const { defaultLevel, ch } = await this.getInitData();
      ...
      const res = await axios.post(this.encryptUrl, formData, { headers: this.headers });
      const $ = cheerio.load(res.data);
